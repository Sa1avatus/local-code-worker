...
<script>
const $=id=>document.getElementById(id), provider=$('provider'), baseUrl=$('baseUrl'), model=$('model'), pullModel=$('pullModel'), apiKey=$('apiKey'), status=$('status'), progress=$('progress');let clearKey=false;
...
async function load(){try{const data=await jsonFetch('/api/settings');provider.value=data.provider;baseUrl.value=data.base_url;sync();await models(data.model||'');$('health').textContent=data.api_key_configured?'ключ сохранён':'локальная конфигурация'}catch(e){message(e.message,false)}}
