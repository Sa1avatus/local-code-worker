...
function fillTier(name,data){$(name+'Enabled').checked=data.enabled;$(name+'Provider').value=data.provider;$(name+'BaseUrl').value=data.base_url||'';setTierModel(name,data.model);$(name+'Context').value=data.context_length||32768;$(name+'Parallel').value=data.num_parallel;$(name+'Think').value=data.think===true?(data.show_reasoning===false?'hide':'show'):(data.think===false?'off':'');$(name+'Key').value='';$(name+'KeyState').textContent=data.api_key_configured?'Ключ сохранён':'Ключ не задан'}
function tierPayload(name){const key=$(name+'Key').value,clear=clearedTierKeys.has(name),action=clear?'clear':key?'replace':'keep';const tv=$(name+'Think').value,think=(tv==='show'||tv==='hide')?true:(tv==='off'?false:null),show_reasoning=(tv==='show')?true:(tv==='hide'?false:null);return {enabled:$(name+'Enabled').checked,provider:$(name+'Provider').value,base_url:$(name+'BaseUrl').value,model:$(name+'Model').value,context_length:Number($(name+'Context').value),num_parallel:Number($(name+'Parallel').value)||1,think:think,show_reasoning:show_reasoning,api_key_action:action,api_key:action==='replace'?key:null}}
